...
**Answer: The observations are taken at the household level. A discrepancy in the data is that round 1 has seven fewer observations than round 2.**
...
**Answer: The underlying migration rate in the control group is 0.360**
...
stargazer(list(model_consumption), title="Regression of Migrant on Treatment Arm Indicators", type="text", se = list(model_consumption$rse))
```
...
stargazer(list(model_consumption_noFE), title="Regression of Migrant on Treatment Arm Indicators", type="text", se = list(model_consumption_noFE$rse))
```
...
stargazer(list(column4), title="Regression of Migrant on Treatment Arm Indicators", type="text", se = list(column4$rse))
```
...
**Answer: First stage is migration regressed on treatment.**
...
first_stage1<-felm(migrant~cash+credit+info|upazila, dataset2)
first_stage2<-felm(migrant~cash+credit+info+num_adltmalesr1+walls_good+subsistencer1+num_childrenr1+bankedr1+constrainedr1+exp_total_pc_r1+dhaka_network+dhaka_remit+monga+lit+avgQ13earned|upazila, dataset2)
first_stage_ITT<-felm(migrant~ITT|upazila, dataset2)

stargazer(list(first_stage1, first_stage2,first_stage_ITT), title="Regression of Migrant on Treatment Arm Indicators", type="text", se = list(first_stage$rse,first_stage2$rse,first_stage_ITT$rse))
...
**Code:**
```{r}
column6<-felm(average_exp2~1|upazila|(migrant~ITT), dataset2)
column7<-felm(average_exp2~num_adltmalesr1+walls_good+subsistencer1+num_childrenr1+bankedr1+constrainedr1+exp_total_pc_r1+dhaka_network+dhaka_remit+monga+lit+avgQ13earned|upazila|(migrant~ITT), dataset2)


stargazer(list(column6, column7), title="Regression of Migrant on Treatment Arm Indicators", type="text", se = list(column6$rse,column7$rse))

```


**Answer: The coefficient of column 6 can be interpreted to mean that migrating causes consumption to increase by 374.857. The coefficient of column 7 can be interpreted to mean that after controlling for various household characteristics migrating causes and increase in consumption of 333.378. **
...
**Answer: This value is particularly relevant for policy decisions in the context of this experiment because it means that if you could induce migration for individuals living in rural villages you could then increase their household consumption by over 300. This means that inducing migration would be economically beneficial and therefore soemthing policy makers would want to consider.**
...
**Answer: I think this particular project definitely has an external validity problem, because there are a lot of ways that general equilibrium effects would change the outcome of this experiment if it were performed on a larger scale. Given the extra article that **
